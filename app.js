...
function shell(content) {
  return `<div class="app-shell"><header class="topbar">${brand}<span class="step-count">Step ${state.step} / 5</span></header><div class="progress" aria-hidden="true"><span style="width:${state.step * 20}%"></span></div>${content}</div>`;
}
...
function renderProfile() {
  ...
  </div><div class="actions split"><button class="btn btn-ghost" data-action="back">返回</button><button class="btn btn-primary" data-action="to-test">开始测试<span>→</span></button></div></section>`);
}
...
app.addEventListener('click', async event => {
  ...
  if (action.dataset.action === 'back-result') { state.step = 4; render(); }
  ...
  if (action.dataset.action === 'download' && state.badgeBlobUrl) {
    ...
    link.download = `DCDC-${state.profile.name}-${state.result.primary.nameEn}.png`;
