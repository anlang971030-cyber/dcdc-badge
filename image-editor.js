...
export function selectImageFlow(flow) {
  ...
  imageState.error = flow === 'transparent'
    ? '透明背景素材模式：准备上传多张 PNG 素材并进行排版。'
    : 'ASCII 模式：上传照片后生成字符风格图片。';
}
...
export function editorMarkup() {
  ...
  if (!s.flow) {
    return `<section class="screen">
      <p class="eyebrow">IMAGE WORKFLOW</p>
      <h2>选择图片制作方式</h2>
      <div class="mode-card-grid">
        <button class="mode-card" data-action="select-image-flow" data-flow="transparent">
          <strong>上传心仪透明背景图</strong>
          <span>支持多张透明图片上传、排版和缩放。</span>
        </button>
        <button class="mode-card" data-action="select-image-flow" data-flow="ascii">
          <strong>上传照片生成 ASCII 风格图</strong>
          <span>保留原 ASCII 参数、颜色和生成流程。</span>
        </button>
      </div>
      <button class="btn btn-ghost" data-action="choose-mode">返回制作方式</button>
    </section>`;
  }
