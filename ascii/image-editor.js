...
export function editorMarkup() {
  ...
  <div class="actions ascii-top-actions"><button class="btn btn-primary" data-action="convert-image" ${imageState.busy || !imageState.activeSourceId ? 'disabled' : ''}>${imageState.busy ? '正在处理…' : '生成当前图片 ASCII'}</button><button class="btn btn-ghost" data-action="download-art" ${!activeArtwork()?.artworkUrl ? 'disabled' : ''}>下载当前透明字符画 PNG</button><button class="btn btn-ghost" data-action="choose-mode">返回制作方式</button></div>
